...
public class PDFAExtensionSchema extends XMPSchema {
	...
	public PDFAExtensionSchema(XMPMetadata metadata) {
		...
		setAttribute(new Attribute(NS_NAMESPACE, "xmlns",
				PDFASCHEMA, PDFASCHEMAURI));
		setAttribute(new Attribute(NS_NAMESPACE, "xmlns",
				PDFAPROPERTY, PDFAPROPERTYURI));
		setAttribute(new Attribute(NS_NAMESPACE, "xmlns",
				PDFATYPE, PDFATYPEURI));
		setAttribute(new Attribute(NS_NAMESPACE, "xmlns",
				PDFAFIELD, PDFAFIELDURI));
	 ...
	 */
	public PDFAExtensionSchema(XMPMetadata metadata, Map<String, String> namespaces) throws XmpSchemaException {
		super(metadata, PDFAEXTENSION, PDFAEXTENSIONURI);
		if(!namespaces.containsKey(PDFAEXTENSION)){
			throw new XmpSchemaException("Extension schema is declared without the pdfaSchema namespace specification");
		}
		namespaces.remove(PDFAEXTENSION);
		
		for (Entry<String, String> entry : namespaces.entrySet()) {
		  setAttribute(new Attribute(NS_NAMESPACE,"xmlns",entry.getKey(),entry.getValue()));
		}
		descriptions=new SchemaDescriptionContainer();
		getElement().appendChild(descriptions.getElement());
	...
	/**
	 * Add to the Extension Schema a new description schema
	 * param desc the schema description
	 * @param desc the new schema description
	...
	public class SchemaDescriptionContainer implements Elementable {
		...
		protected List<SchemaDescription> schemaDescriptions;
			...
			public SchemaDescriptionContainer() {
				element=metadata.getFuturOwner().createElement(PDFAEXTENSION+":schemas");
...
//				}
			...
			protected SchemaDescription getSameSchemaDescription (SchemaDescription obj) {
...
//			 * @return
//			 */
//			public boolean isSameSchemaDescription(SchemaDescription prop1, SchemaDescription prop2){
