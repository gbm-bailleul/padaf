...
import net.padaf.xmpbox.parser.XMLUtil;
...
public class SaveMetadataHelper {
	 ...
	 * @param intoXPacket, true if Processing instruction must be embedded
	 */
	protected static Document prepareSaving(XMPMetadata metadata, boolean intoXPacket) {
		...
		if(intoXPacket){
			ProcessingInstruction beginXPacket = newdoc.createProcessingInstruction("xpacket", "begin=\""
					+ metadata.getXpacketBegin() + "\" id=\"" + metadata.getXpacketId() + "\"");
			newdoc.appendChild(beginXPacket);
		}
		
		Element xmpMeta = newdoc.createElementNS("adobe:ns:meta/",
				"x:xmpmeta");
		...
		if(intoXPacket){
			ProcessingInstruction endXPacket = newdoc.createProcessingInstruction("xpacket", metadata.getEndXPacket());
	...
	/**
	 * Serialize in an output stream with XPacket PI
	 * @param metadata
     * @param os
	 * @throws TransformException
	 */
	public static void serialize(XMPMetadata metadata, OutputStream os) throws TransformException {
	 ...
	 * @param metadata
     * @param intoXPacket True to put XPacket Processing Information 
	 * @param os
	 * @throws TransformException
	 */
	public static void serialize(XMPMetadata metadata, boolean intoXPacket, OutputStream os) throws TransformException {
	 ...
	 * @throws TransformException
	 */
	public static byte[] serialize(XMPMetadata metadata) throws TransformException {
		return serialize(metadata, true);
	}
	
	/**
	 * Serialize a schema into an outputstream 
	 * @param schema
	 * @param os
	 * @throws TransformException
	 * @throws IOException 
	...
	public static void serialize(XMPSchema schema, OutputStream os) throws TransformException{
		try {
			...
			throw new TransformException("Failed to create Document to contain Schema representation ", e.getCause());
		}

	}
	
	/**
	 * Serialize a schema into the returned byte array
	 * @param schema
	 * @param os
	 * @throws TransformException
	 */
