...
public class PDFAExtensionTest {
  ...
  @Test
  public void testNSManualDeclaration() throws XmpSchemaException{
    HashMap<String, String> namespaces=new HashMap<String, String>();
    namespaces.put(PDFAExtensionSchema.PDFAEXTENSION, PDFAExtensionSchema.PDFAEXTENSIONURI);
    namespaces.put(PDFAExtensionSchema.PDFAFIELD, PDFAExtensionSchema.PDFAFIELDURI);
    namespaces.put(PDFAExtensionSchema.PDFAPROPERTY, PDFAExtensionSchema.PDFAPROPERTYURI);
    namespaces.put(PDFAExtensionSchema.PDFASCHEMA, PDFAExtensionSchema.PDFASCHEMAURI);
    namespaces.put(PDFAExtensionSchema.PDFATYPE, PDFAExtensionSchema.PDFATYPEURI);

    PDFAExtensionSchema schem=metadata.createAndAddPDFAExtensionSchemaWithNS(namespaces);
    Iterator<Attribute> att=schem.getAllAttributes().iterator();

    //PDFAExtension is removed during the building of PDFAExtensionSchemaWithNS
    namespaces.put(PDFAExtensionSchema.PDFAEXTENSION, PDFAExtensionSchema.PDFAEXTENSIONURI);
    Attribute tmp;
  ...
  public void testNoPdfExtension () throws Exception {
    ...
    PDFAExtensionSchema schema = new PDFAExtensionSchema(metadata, namespaces);
  }
  ...
  public void testPDFExt() throws Exception{
    ...
    PDFAExtensionSchema schem=metadata.createAndAddPDFAExtensionSchemaWithDefaultNS();


    Assert.assertEquals("pdfaExtension", schem.getPrefix());
    Assert.assertEquals("http://www.aiim.org/pdfa/ns/extension/",schem.getNamespaceValue());
    ...
    desc.addProperty("nom", "Text", "external", "nom de la personne concernee");
    desc.addProperty("nom", "Text", "external", descExpected);
    Assert.assertEquals(1, desc.getProperties().size());
    Assert.assertEquals(descExpected, desc.getProperties().get(0).getDescriptionValue());
    desc.addProperty("prénom", "Text", "external", "prénom de la personne concernée");
    ...
    desc2.addProperty("TestText", "OwnType", "external", "just a text property");
