...
public class BirthCertificateSchemaWithXMLDescriptions extends XMPSchema {
  ...
  public BirthCertificateSchemaWithXMLDescriptions(XMPMetadata metadata) {
    ...
    this.setAttribute(new Attribute(null, "xmlns", "madn", "http://test.withfield.com/vt/"));
  ...
  public String getFirstname ()  {
    return this.getTextProperty(localPrefixSep+FIRST_NAME).getStringValue();
  }

  public String getLastname () {
    return this.getTextProperty(localPrefixSep+LAST_NAME).getStringValue();
  }

  public void setMailaddr(String name, String domain) {
	  ComplexPropertyContainer field=new ComplexPropertyContainer(metadata, localPrefix, MAIL_ADR);
  ...
  private ComplexPropertyContainer getMailField() {
	  ...
	  if(afield instanceof ComplexPropertyContainer){
		 return (ComplexPropertyContainer) afield;
		  ...
		  throw new IllegalArgumentException(MAIL_ADR+" property found but not seems to be a field");
	  }
  }
	  ...
	  while(it.hasNext()){
		  ...
		  if(aProp.getPropertyName().equals(nameProp)){
			  if(aProp instanceof TextType){  
				  ...
				  throw new IllegalArgumentException(nameProp+" property found but not seems to be in expected type");
			  }
		  }
	  }
	 return null;
