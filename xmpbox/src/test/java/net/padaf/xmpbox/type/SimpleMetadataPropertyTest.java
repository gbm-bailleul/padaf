...
public class SimpleMetadataPropertyTest {
	...
	public void testElementAndObjectSynchronization() throws Exception{
		...
		IntegerType integer= new IntegerType(parent, "test", "integer", integerv);
		RealType real= new RealType(parent, "test", "real", realv);
		TextType text= new TextType(parent, "test", "text", textv);
		
		Assert.assertEquals(bool.getNamespace(), bool.getElement().getNamespaceURI());
		Assert.assertEquals(bool.getPrefix()+":"+bool.getPropertyName(), bool.getElement().getNodeName());
		Assert.assertEquals(bool.getQualifiedName(), bool.getElement().getNodeName());
	...
	public void testObjectCreationFromJavaType() throws Exception{
		...
		DateType date= new DateType(parent, "test", "date", Calendar.getInstance());
	...
	public void testObjectCreationWithNamespace() throws Exception{
			...
			DateType date= new DateType(parent, ns, "test", "date", Calendar.getInstance());
	...
	public void testAttribute() throws Exception{
		...
		IntegerType integer= new IntegerType(parent, "test", "integer", 1);
		Attribute value=new Attribute( "http://www.test.org/test/", "test", "value1", "StringValue1");
		...
		Assert.assertEquals(value, integer.getAttribute(value.getQualifiedName()));
		Assert.assertTrue(integer.containsAttribute(value.getQualifiedName()));
		
		//Replacement check
		
		integer.setAttribute(value2);
		Assert.assertEquals(value2, integer.getAttribute(value2.getQualifiedName()));
	
		integer.removeAttribute(value2.getQualifiedName());
		Assert.assertFalse(integer.containsAttribute(value2.getQualifiedName()));
		
		//Attribute with namespace Creation checking
		Attribute valueNS=new Attribute("http://www.tefst2.org/test/", "test2", "value2", "StringValue.2");
		integer.setAttribute(valueNS);
		Attribute valueNS2=new Attribute("http://www.test2.org/test/", "test2", "value2", "StringValueTwo");
